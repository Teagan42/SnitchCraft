...
func TestStartProxyServer_Forbidden(t *testing.T) {
	...
	// Start server
	server := httptest.NewServer(GetHandler(cfg))
	defer server.Close()
...
func TestStartProxyServer_ProxySuccess(t *testing.T) {
	...
	// Start proxy server
	proxy := httptest.NewServer(GetHandler(cfg))
	defer proxy.Close()

	req, _ := http.NewRequest("GET", proxy.URL, nil)
	req.Header.Set("Host", "example.com") // set a host header to avoid 400 Bad Request
